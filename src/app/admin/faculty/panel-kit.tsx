...
/**
 * The pieces the faculty screen shares.
 *
 * A near-copy of `site-settings`, `home`, `about`, `academics` and
 * `admission`'s files of the same name, for the reason M5 states in its own
 * opening line: each module is independently shippable, which a module
 * importing its form furniture from a sibling route is not. The shared home is
 * `src/components/admin/**`, which belongs to T-051 and which no card in this
 * batch may touch.
 */
...
import type { ActionResult } from "@/lib/modules/faculty/result";
...
export type TextFieldProps = {
  ...
  disabled?: boolean;
  error?: string;
  type?: "text" | "number" | "date" | "time" | "datetime-local" | "url" | "email" | "tel";
  hint?: string;
  placeholder?: string;
  lang?: string;
};
...
/** A group of checkboxes over a fixed option list — used for the subjects multi-select. */
export function CheckboxGroup({
  ...
  function toggle(code: string, checked: boolean) {
    onChange(
      checked ? [...selected, code] : selected.filter((entry) => entry !== code),
    );
  }
  ...
  return (
    <fieldset className="mb-4 border-0 p-0">
      ...
      <div className="flex flex-wrap gap-x-6 gap-y-2">
        {options.map((option) => (
          <label key={option.code} className="flex items-center gap-2 text-body">
            <input
              type="checkbox"
              className="h-4 w-4"
...
/** A `SMALLINT` field's value on its way back to the schema. See `admission/panel-kit.tsx`. */
...
/** A `type="date"` input's value, to the `TIMESTAMPTZ` schema `optionalTimestamp` expects. */
