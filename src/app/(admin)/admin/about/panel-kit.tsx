 ...
 * 422 on `publishConsentAt`, so a panel that swallowed refusals would report a
 * committee member as published who is not.
 *
 * A near-copy of the `site-settings` and `home` files of the same name, for the
 * reason M5 states in its own opening line: each module is independently
 * shippable, which a module importing its form furniture from a sibling route
 * is not. The shared home is `src/components/admin/**`, which belongs to T-051
 * and which no card in this batch may touch.
 */
...
import type { Copy } from "@/app/(admin)/admin/about/copy";
import type { ActionResult } from "@/lib/modules/about/result";
