/**
 * A Server Action's return value, for the `academics` module.
 *
 ...
 *
 * This card's Contract makes that unusually load-bearing. "Deleting a class
 * grade with dependent fee structures or exams is refused with an explanation"
 * is a promise about a *message* — the refusal has to say which fee structures
 * and which exams are in the way, or the admin is left guessing at a button
 * that does nothing. So refusals travel as data, and the naming happens in
 * `actions.ts` where the blocking rows are actually in hand.
 *
 ...
 *
 * The fourth near-identical copy (`site-settings`, `home`, `about`, and now
 * this), and deliberately so: M5 opens by requiring every module to be
...
export type ActionFailure = {
  ...
  /** Populated for a 422 only; `field` is the schema path, e.g. `values.endsOn`. */
