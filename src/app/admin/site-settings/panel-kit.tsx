 ...
 * re-read itself, and say so in a toast. Written per panel that is five copies
 * of the same reasoning and five chances for one of them to swallow a refusal
 * silently — which is the failure mode that matters here, because a save that
 * looks like it worked is worse than one that visibly did not.
 *
 * The rest is plain form furniture. `PermissionGate` (T-051) is presentation
 * only by contract, so nothing here is a security control: an admin who edits
 * the DOM to re-enable a disabled input still meets stage 2 of the write
 * pipeline on the server, which is the check that decides.
 */
...
import type { ActionResult } from "@/lib/modules/site-settings/result";
