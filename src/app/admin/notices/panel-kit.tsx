...
/**
 * The pieces the notices screen shares.
 *
 * A near-copy of `site-settings`, `home`, `about`, `academics`, `admission`
 * and `faculty`'s files of the same name — see `admission/panel-kit.tsx` for
 * why it stays a copy: each M5 module is independently shippable.
 *
 * `slugify` is the one addition no earlier module needed: §B-11's slug is
 * per-locale and admin-editable, but the Do line asks for "auto-generation",
 * so `NoticesPanel` derives it from the title and only stops updating it once
 * the admin has touched the slug field directly.
 */
...
import type { ActionResult } from "@/lib/modules/notices/result";
import type { FieldIssue } from "@/lib/mutate";

/**
 * What the signed-in admin may do in this module.
 *
 * `publish` is its own boolean — §A-5.2 grants `notice` five actions, and
 * add/edit no longer imply the right to put a notice on the site.
 */
export type Rights = {
  add: boolean;
  edit: boolean;
...
export function anyRight(rights: Rights): boolean {
  return rights.add || rights.edit || rights.delete || rights.publish;
}
...
/**
 * A URL slug from free text — lowercase, hyphenated, Unicode-aware, matching
 * `primitives.ts`'s `SLUG_PATTERN`.
 */
export function slugify(text: string): string {
  return text
    .trim()
    .toLowerCase()
    .replace(/[^\p{Ll}\p{Lo}\p{N}\p{M}]+/gu, "-")
    .replace(/^-+|-+$/g, "");
}
