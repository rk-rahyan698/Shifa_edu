/**
 * A Server Action's return value, for the `home` module.
 *
 ...
 * stripped out in production. This card's Contract — a slide whose image has no
 * Bangla alt text is refused — is only useful if the admin is told *which*
 * field and *why*, so refusals travel as data.
 *
 ...
 *
 * **Yes, this is the third copy** — `site-settings`, `home` and `about` each
 * carry it, and each is a near-duplicate of the others. That is deliberate
 * rather than overlooked. M5 opens by requiring every module to be
 * "independently shippable", and a `home` module that stops compiling when
 * `site-settings` is reverted is not. The shared home for this is a
 * `src/lib/modules/result.ts` that no card in this batch owns; until a card
 * does, three small copies beat one shared file three tasks are forbidden to
 * touch.
 */
