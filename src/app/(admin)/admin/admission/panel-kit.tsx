 ...
 *
 * A near-copy of the `site-settings`, `home`, `about` and `academics` files of
 * the same name, for the reason M5 states in its own opening line: each module
 * is independently shippable, which a module importing its form furniture from
 * a sibling route is not. The shared home is `src/components/admin/**`, which
 * belongs to T-051 and which no card in this batch may touch.
 *
 * Two things here are not in the earlier three, and both come from §A-5.2
 * giving `admission` four actions rather than the two `about` had:
 *
 ...
 *    that hides the add button from someone holding `admission:add` is a screen
 *    that misreports what they may do.
 *  - `ListRow`, because four of this screen's six panels are the same list of
 *    rows with the same two links. The fee grid is the one that is not, and it
...
import type { ActionResult } from "@/lib/modules/admission/result";
