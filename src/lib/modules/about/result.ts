/**
 * A Server Action's return value, for the `about` module.
 *
 ...
 * stripped out in production. This card's Contract — a committee member cannot
 * be activated without recorded consent — is only useful if the admin is told
 * which field and why, so refusals travel as data.
 *
 ...
 *
 * The third of three near-identical copies (`site-settings`, `home`, `about`),
 * and deliberately so: M5 opens by requiring every module to be "independently
 * shippable", which a module that stops compiling when a sibling is reverted is
 * not. The shared home is a `src/lib/modules/result.ts` that no card in this
 * batch owns.
 */
